...
def load_sd_table(seasons):
    seasons_str = "(" + seasons + ")"
    df = load_db_to_pd(sql_query="SELECT * FROM [Development].[dbo].[sd_tableF] where seasonId in %s" % seasons_str,
                       db_name='Development')
    return df

def load_pos_data(seasons):
    seasons_str = "(" + seasons + ")"
    df_possesion_sequences_ordered = load_db_to_pd(
        sql_query="SELECT t.* , me.playerId, m.seasonId, me.teamId, p.possessionId, p.possessionEventIndex, v.sumVaep "
                  "FROM [Scouting_Raw].[dbo].[Wyscout_Events_Possession] as p "
                  "JOIN Scouting_Raw.dbo.Wyscout_Events_TypeSecondary as t ON t.eventId = p.eventId "
                  "JOIN Scouting.dbo.Wyscout_Matches AS m ON m.matchId = t.matchId "
                  "join scouting_raw_staging.dbo.Vaep as v on t.eventId = v.eventId "
                  "join Scouting_Raw.dbo.Wyscout_Events_Main_Info as me on me.eventId = t.eventId "
                  "WHERE m.seasonId in %s ORDER BY p.possessionId ASC, possessionEventIndex" % seasons_str,
        db_name='Scouting_Raw')
